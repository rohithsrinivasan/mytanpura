...
export function taper(t, lo, hi, curve = 1) {
  const x = clamp01(t);
...
export function smoothingCoef(seconds, sampleRate, blockSize = 1) {
...
export const MIN_SA = 36;
export const MAX_SA = 72;
/** C#3, the classic male-voice tanpura pitch. */
export const DEFAULT_SA = 49;

export function frequency(midi, cents = 0, a4Hz = 440) {
  return a4Hz * Math.pow(2, (midi - 69 + cents / 100) / 12);
}

export function frequencyFromSemitone(semitonesFromSa, saMidi, cents, a4Hz) {
 ...
 * value.
 */
export function perString(values, count) {
...
export function effectiveReverbMix(settings) {
  return settings.reverbMix < 0 ? voiceById(settings.voiceId).reverbMix : settings.reverbMix;
}

export function stringGain(settings, index) {
...
export function normalizedCorr(samples, offset, length, mean, lag) {
  let corr = 0;
  let normA = 0;
  let normB = 0;
...
export function detectPitch(samples, sampleRate, minHz = 50, maxHz = 1200, offset = 0, length = samples.length - offset) {
